...
Check: load `references/nextjs_defaults.md` to understand the technical baseline.
...
2. **Scan the codebase for context** (silent):
   ...
   - Scan `src/features/` for related code
   - Scan `specs/cr/` for related CRs already in progress
   - Load `references/nextjs_defaults.md`
...
3. **Form an initial read** on each issue:
   ...
   - What does it touch? (which feature, which layers, which routes)
