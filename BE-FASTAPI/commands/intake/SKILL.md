...
- Plain text description: "the login endpoint is slow"
...
Check: load `.claude/references/unified_workflow.md` to understand the full process.
...
2. **Scan the codebase for context** (silent):
   ...
   - Scan `src/` or `lib/features/` for related code
   - Scan `specs/cr/` for related CRs already in progress
   - Load `.claude/references/technical_defaults.md` and `hexagonal_architecture.md`
...
3. **Form an initial read** on each issue:
   ...
   - What does it touch? (which domain, which layers, which tenants)
...
Security impact: does it touch auth, tenant isolation, data access
...
2. [Question 2 — only if genuinely unanswerable from context]
