...
- Plain text description: "the document processing task is failing silently"
...
- If `specs/project.md` exists → read it. This is the primary context source. Do NOT scan `src/` unless needed to locate a specific file.
- If `specs/project.md` does NOT exist → tell the developer: "No project context found. Run `/init` first to set up the project context. This helps me understand your modules without scanning the whole codebase every time."
...
2. **Load context** (silent — in this order, stop when you have enough):
   - Read `specs/project.md` if it exists → this tells you what modules exist and where they live
   - Read `specs/cr/` for related CRs already in progress
   - Read `references/technical_defaults.md` for technical baseline
   - **Only read `src/`** if the issue mentions a specific module and `specs/project.md` doesn't have enough detail to locate it
...
3. **Form an initial read** on each issue:
   ...
   - What does it touch? (which module from the project map, which layers, which domain)
   - What is the blast radius?
   - Is it reversible?
   - Are there dependencies on other modules or CRs?
...
|------|-----------|-------|
| `feature` | New capability, new endpoint, new domain operation | Full: spec → plan → build → close |
| `bug` | Existing behavior is broken or wrong | Minimal: locate → fix → verify (no full spec, no plan) |
| `change` | Small modification to existing behavior (limit, config, copy, threshold) | Lean: 3-section spec → build → close |
| `security` | Auth gap, data exposure, secrets, injection risk, tenant isolation breach | Security track: threat model → fix → verify |
| `incident` | Service down or degraded, data at risk | Incident track: containment first → fix → close |
...
Security impact: does it touch auth, tenant isolation, data access, secrets
...
- Security: [flag if auth, tenant isolation, or data access is involved]
...
### If multiple CRs were created from a single intake (e.g. findings report):
- Create one `.cr.md` file per CR item
- Create an additional `specs/cr/<date>-intake-summary.md` listing all CRs, their priorities, and recommended sequence
