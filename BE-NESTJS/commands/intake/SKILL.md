...
- If `specs/project.md` exists → read it. This is the primary context source. Do NOT scan `src/modules/` unless needed to locate a specific file.
...
2. **Load context** (silent — in this order, stop when you have enough):
   ...
   - Read `references/nestjs_defaults.md` for technical baseline
   - **Only read `src/modules/`** if the issue mentions a specific module and `specs/project.md` doesn't have enough detail to locate it
...
3. **Form an initial read** on each issue:
   ...
   - What does it touch? (which module from the project map, which layers, which tenants)
...
|------|-----------|-------|
| `feature` | New capability, new module, new endpoint | Full: spec → plan → build → close |
...
| `security` | Auth gap, RLS breach, data exposure, secrets, OIDC issue | Security track: threat model → fix → verify |
