...
|---------|-------|--------------|
| `/init` | Setup | One-time project setup. Creates specs/project.md, scaffolds lib/. Run once before anything else. |
...
| `/code-review` | Discovery | Multi-agent audit of existing Flutter code. Produces findings report. |
...
| `bug` | Existing behavior is broken or crashes | `/intake` → `/build` (locate → regression test → fix) → `/close` |
| `change` | Small modification — copy, style, config, limit | `/intake` → `/spec` (3 sections) → `/build` → `/close` |
| `security` | Auth gap, token storage, user data exposure | `/intake` → `/spec` → `/plan` → `/build` → `/close` |
...
test/features/<f>/                  ← TDD tests written before code (created by /plan)
