...
- Plain text description: "the profile screen crashes when the user has no avatar"
...
- If `specs/project.md` exists → read it. This is the primary context source. Do NOT scan `lib/features/` unless needed to locate a specific file.
...
2. **Load context** (silent — in this order, stop when you have enough):
   ...
   - Read `references/flutter_defaults.md` for technical baseline
   - **Only read `lib/features/`** if the issue mentions a specific feature and `specs/project.md` doesn't have enough detail to locate it
...
3. **Form an initial read** on each issue:
   ...
   - What does it touch? (which feature from the project map, which layers, which screens)
...
| `change` | Small modification to existing behavior (copy, config, style, limit) | Lean: 3-section spec → build → close |
| `security` | Auth gap, token storage issue, user data exposure, injection risk | Security track: threat model → fix → verify |
| `incident` | Production crash or data corruption | Incident track: containment first → fix → close |
| `refactor` | Internal restructure, no behavior change | Lean: bounded context + ACs only → build → close |

**Severity:**
```
Critical → production crash NOW, data at risk
...
Security impact: does it touch auth, token storage, user isolation, permissions
...
- Security: [flag if auth, token storage, or user isolation is involved]
